...
export async function generateRAGAnswer(params: {
  ...
  if (ragMode === 'NONE') {
    effectiveSystemInstruction += `\n\n--- [СТРОГИЕ ПРАВИЛА RAG И ОТСУТСТВИЕ ДОКУМЕНТОВ] ---
1. В ВАШЕЙ ВЕКТОРНОЙ БАЗЕ ЗНАНИЙ (RAG) СЕЙЧАС НЕТ ЗАГРУЖЕННЫХ ДОКУМЕНТОВ КОМПАНИИ.
2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО придумывать вымышленные факты, биографии или регламенты о компании KRB.
3. ВЫ ОБЯЗАНЫ НАЧИНАТЬ СВОЙ ОТВЕТ СТРОГО С СЛЕДУЮЩЕЙ СТРОКИ:
⚠️ В Библиотеке бизнеса KRB пока нет официальных документов по данному вопросу.

После этой строки, если пользователь задает общий вопрос, давай полезный ответ на основе живого поиска в интернете (Google Search Grounding), не выдавая выдумки за факт.`;
  } else if (ragMode === 'DUAL_RAG') {
...
Не принимай данные из документов за абсолютную истину — проверяй их на логическую согласованность.`;
  ...
  // Общее правило чистоты ответа без технического мусора и галлюцинаций
  effectiveSystemInstruction += `\n\n--- [ПРАВИЛА ОФОРМЛЕНИЯ И ФАКТОВ] ---
КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО придумывать несуществующие биографии, вымышленные истории и неверные данные.
